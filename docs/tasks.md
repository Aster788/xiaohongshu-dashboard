...
**当前状态（一行）**：**阶段 1–6 已落地；阶段 7 的主题、Logo、部署文档与生产验证已完成**（Vercel + Neon + `UPLOAD_SECRET` 已完成一次端到端：部署、`prisma migrate deploy`、线上上传与首页数据刷新）；待完成可选 Windows build 稳定性收尾。
...
3. ✅ 环境变量与生产数据库、密钥配置文档（不含真实值）。
  落地：`docs/vercel-deploy-runbook.md` 已覆盖 Vercel 部署、Neon 连接、`DATABASE_URL` / `UPLOAD_SECRET`、`prisma migrate deploy`、smoke test 与 rollback；并已完成一次线上上传 → 数据入库 → `/` 反映变化的生产验证。
